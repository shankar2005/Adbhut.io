...
const WorkDemo = ({ demo_type, demo_link }) => {
    ...
    switch (demo_type) {
        ...
        case "Instagram":
            content = (
                <a target="_blank" href={demo_link}>
                    <div className="demo-square !bg-white gap-y-4">
                ...
                <>
                    {
                        (demo_link?.includes("drive.google.com") && demo_link?.includes("/folders/"))
                            ? <iframe className="border bg-gray-100" src={`https://drive.google.com/embeddedfolderview?id=${extractFolderId(demo_link)}#grid`} width="100%" height={600} scrolling="no"></iframe>
                            : <iframe className="border" src={demo_link.replace("/view", "/preview")} width="100%" height={600} allow="autoplay"></iframe>
                    }
                </>
            )
            break;
        case "Behance":
            content = (
                <a href={demo_link} className='demo-square' target="_blank">
                    ...
                    <p className='font-medium text-white flex justify-center gap-2 mt-2'>Open in behance <BsBoxArrowUpRight /></p>
                </a>
            )
            break;
        case "Imdb":
